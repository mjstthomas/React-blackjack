...
class App extends React.Component {
  ...
  manageSignIn = (obj) => {
    firebase.auth().onAuthStateChanged((user) => {
      ...
      fetch(
        `${config.API_ENDPOINT}/users/${userProf.email}:${userProf.id}:${userProf.displayName}`
      )
  ...
  handleImage = () => {
    if (this.state.user.image) {
      const userImage = images.find(
        (item) => this.state.user.image === item.title
      );
  ...
  render() {
    ...
    return (
      <AppContext.Provider value={context}>
        <div className="App">
          <Header user={this.state.user} handleDemo={this.handleDemo} />
